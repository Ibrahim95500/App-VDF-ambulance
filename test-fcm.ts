...
async function testFCM() {
    ...
    if(!token) {
        console.error("Please supply a token as an argument");
        process.exit(1);
    }

    console.log(`Sending test notification to token: ${token.substring(0,25)}...`);

    const payload: admin.messaging.MulticastMessage = {
        tokens: [token],
        notification: {
            title: "Test Antigravity",
            body: "Ceci est un test direct natif",
        },
        android: {
            priority: 'high',
            notification: {
    ...
    try {
        ...
        if(response.failureCount > 0) {
            console.dir(response.responses[0].error, {depth: null});
        }
    } catch(e) {
